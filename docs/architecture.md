...
4. **Aktör izolasyonu:** webhook'ta SecurityContext yok; `updateTaskStatusAsSystem` erişim kontrolünü atlar, aktörü explicit alır. `resolveGitActor(GitRef)` commit/MR yazarının email'ini `User.email` ile (case-insensitive) eşler; bulunamazsa `git-otomasyonu` sistem kullanıcısını döner.

...
Yönetim Paneli'nde **"Git Entegrasyonu"** sekmesi: aç/kapa toggle, webhook secret input (boş = değiştirme; `secretConfigured` rozetli), 3 durum-senkron dropdown (MR açıldı / MR merge / **branch açılınca**); push otomatik durum uygulamaz. Webhook Secret alanında **"Üret"** butonu (tarayıcıda `crypto.getRandomValues` ile 32-byte hex). Platform başına webhook URL gösterimi (`/api/webhooks/git/{github|gitlab|gitea}`). `GET/PUT /api/admin/git/settings` (`api` instance). Status seçenekleri `TaskStatus` enum'uyla eşleşir (`OPEN`, `IN_PROGRESS`, `TESTING`, `COMPLETED`, `CANCELLED`).
